...
export function classifyMergeError(err: any): 'stale-base' | 'real-conflict' | 'other' {
  if (err?.status !== 405) return 'other';
  const message: string = err?.response?.data?.message ?? err?.message ?? '';
  if (/base branch was modified/i.test(message)) return 'stale-base';
  if (/merge conflict/i.test(message)) return 'real-conflict';
  return 'other';
}
...
async function mergeOnce(
  ...
  for (let attempt = 1; attempt <= MAX_STALE_RETRIES; attempt++) {
    try {
      ...
      if (kind === 'real-conflict') return 'real-conflict';
      throw err;
    }
  }
...
async function updateBranchAndWaitUntilSettled(
  octokit: Octokit,
  owner: string,
  repo: string,
  pullNumber: number,
...
export async function mergePullRequest(opts: {
  ...
  for (let round = 0; round <= MAX_UPDATE_ATTEMPTS; round++) {
    ...
    if (round === MAX_UPDATE_ATTEMPTS) {
      throw new Error(
        `PR #${opts.pullNumber} has real merge conflicts with base that didn't resolve after ` +
          `${MAX_UPDATE_ATTEMPTS} branch-update attempt(s). Leaving the PR and branch open for manual resolution.`,
      );
    }
    console.log(
      `PR #${opts.pullNumber} has a real conflict against base (round ${round + 1}/${MAX_UPDATE_ATTEMPTS}) — ` +
